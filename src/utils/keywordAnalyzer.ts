...
const calculateTitleFactors = (
  ...
  const keywordRelevance =
    (userTitleWords.filter((word) => topKeywords.includes(word)).length /
      topKeywords.length) *
    100;
  ...
  const keywordPlacement = totalImportantWords
    ? (importantWordsAtStart / totalImportantWords) * 100
    : 0;
...
const calculateDescriptionFactors = (
  ...
  const keywordCoverage =
    (userDescWords.filter((word) => topKeywords.includes(word)).length /
      topKeywords.length) *
    100;
  ...
  const keywordPlacement = totalImportantWords
    ? (importantWordsInFirst150 / totalImportantWords) * 100
    : 0;
...
const generateRecommendations = (
  ...
  if (titleFactors.keywordRelevance < 70) {
    recommendations.title.push({
      priority: "high",
      message: `Consider adding these popular keywords: ${topKeywords
        .slice(0, 3)
        .join(", ")}`,
    });
  }
  ...
  if (titleFactors.uniquenessScore < 70) {
    recommendations.title.push({
      priority: "medium",
      message:
        "Your title is too similar to existing videos, try making it more unique",
    });
  }
  ...
  if (descFactors.keywordCoverage < 70) {
    recommendations.description.push({
      priority: "high",
      message: `Include these trending keywords: ${topKeywords
        .slice(0, 3)
        .join(", ")}`,
    });
  }
  ...
  if (descFactors.ctaScore < 100) {
    recommendations.description.push({
      priority: "high",
      message:
        'Add a call-to-action (e.g., "Subscribe for more", "Check out my other videos")',
    });
  }
  ...
  if (descFactors.lengthScore < 70) {
    recommendations.description.push({
      priority: "high",
      message:
        "Adjust description length to 100-200 words for optimal engagement",
    });
  }
